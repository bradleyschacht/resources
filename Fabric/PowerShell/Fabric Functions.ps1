...
function New-FabricWorkspace {
    ...
    if ($Capacity) {
        if ($false -eq (Test-FabricId -String $Capacity)) {
    ...
    Invoke-RestMethod -Uri $Uri -Method POST -Headers $Headers -Body $Body -ConnectionTimeoutSeconds 120 -OperationTimeoutSeconds 120
}
...
function Remove-FabricWorkspace {
    ...
    Invoke-RestMethod -Uri $Uri -Method DELETE -Headers $Headers -ConnectionTimeoutSeconds 120 -OperationTimeoutSeconds 120
}
