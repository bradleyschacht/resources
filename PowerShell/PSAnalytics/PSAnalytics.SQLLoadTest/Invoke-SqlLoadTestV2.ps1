function Invoke-SqlLoadTestV2 {
    ...
    if ($true -eq $RunBatch) {
        try {
            ...
            $CapacityCurrent = Get-FabricCapacity -Capacity $CapacityName
            
            Add-LogEntry -Thread $null -Iteration $null -MessageType "Information" -MessageText ("Capacity current state: {0}" -f $CapacityCurrent.state)
            Add-LogEntry -Thread $null -Iteration $null -MessageType "Information" -MessageText ("Capacity current SKU: {0}" -f $CapacityCurrent.sku)
            ...
            if ($CapacityCurrent.sku -ne $CapacitySize) {
                ...
                if (($CapacityCurrent.state -ne "Paused") -and ($true -eq $PauseOnCapacitySkuChange)) {
                    if ($CapacityCurrent.sku -eq "FT1") {
                        Add-LogEntry -Thread $null -Iteration $null -MessageType "Information" -MessageText ("Fabric trials cannot be paused.")    
                    }
                    else {
                ...
                if ($CapacityCurrent.sku -eq "FT1") {
                    Add-LogEntry -Thread $null -Iteration $null -MessageType "Information" -MessageText ("Fabric trials cannot be scaled.")    
                }
                else {
                    Add-LogEntry -Thread $null -Iteration $null -MessageType "Information" -MessageText ("Initiating a scale operation.")
                    $null = Set-FabricAzCapacitySku -SubscriptionID $CapacitySubscriptionID -ResourceGroupName $CapacityResourceGroupName -CapacityName $CapacityName -Sku $CapacitySize
                    $CapacityCurrent = Get-FabricCapacity -Capacity $CapacityName
                }
            ...
            if ($CapacityCurrent.state -ne "Active") {
                # Start the capacity.
