function Invoke-SqlLoadTestV3 {
    param (
        ...
        [Parameter(Mandatory = $false)] [string]  $DataSize,
        [Parameter(Mandatory = $false)] [string]  $DataStorage,
        [Parameter(Mandatory = $false)] [int32]   $ThreadCount             = 1,       <#  Default: 1  #>
        [Parameter(Mandatory = $false)] [int32]   $IterationCount          = 1,       <#  Default: 1  #>
        ...
        # Fabric Data Warehouse and Lakehouse Parameters
        ...
        [Parameter(Mandatory = $false)] [string]  $CapacitySubscriptionID,
        ...
        [Parameter(Mandatory = $false)] [switch]  $AdvancedMode,
        [Parameter(Mandatory = $false)] [switch]  $MetricsOnly,
    ...
    if ($true -eq $AdvancedMode) {
        Write-Host "Advanced mode enabled..." -BackgroundColor "Red" -ForegroundColor "White"
        Start-Sleep -Seconds 2
    }

    if ($true -eq $DebugMode) {
        Write-Host "Debug mode enabled..." -BackgroundColor "Red" -ForegroundColor "White"
        Start-Sleep -Seconds 2
    }
    ...
    $JobInitializationScript = {
        ...
        function Add-LogEntry {
            ...
            elseif ($MessageType -in ("Debug")) {
                Write-Host ("{0} | {1} | Thread {2} | Iteration {3} | Query {4} | {5}" -f $MessageTime, $MessageType, $Thread, $Iteration, $Query, $MessageText) -BackgroundColor "Red" -ForegroundColor "White"
            }
            else {
    ...
    if ($true -eq $MetricsOnly) {
        # Check for the required parameters
        if ($true -eq [string]::IsNullOrEmpty($LogDirectory)) {
            $MetricsOnly = $false
        }
        ...
        $ThreadStatus    = [Hashtable]::Synchronized([ordered]@{})
        $Log             = [Hashtable]::Synchronized([ordered]@{})
        $LogBatch        = [ordered]@{}
        $LogThread       = [Hashtable]::Synchronized([ordered]@{})
        $LogIteration    = [Hashtable]::Synchronized([ordered]@{})
        $LogQuery        = [Hashtable]::Synchronized([ordered]@{})
        $LogQueryError   = [Hashtable]::Synchronized([ordered]@{})
        $LogQueryResult  = [Hashtable]::Synchronized([ordered]@{})
        $LogStatement    = [Hashtable]::Synchronized([ordered]@{})
        $QueryInsights   = [ordered]@{}
        $CapacityMetrics = [ordered]@{}
        ...
        $BatchStartTime = (Get-Date).ToUniversalTime()
    ...
    if ($true -eq $RunBatch) {
        ...
        if ($Platform -eq "Fabric") {
            ...
            # Easy mode setup
            if ($true -eq $RunBatch -and $false -eq $AdvancedMode) {
                # Required parameter: Item Name
                if ($true -eq [string]::IsNullOrEmpty($ItemName)) {
                    $RunBatch = $false
                    Add-LogEntry -Thread $null -Iteration $null -MessageType "Error" -MessageText ("An item name is required to run a Fabric load test.")
                }

                # Required parameter: Server name
                if ($true -eq [string]::IsNullOrEmpty($Server)) {
                    $RunBatch = $false
                    Add-LogEntry -Thread $null -Iteration $null -MessageType "Error" -MessageText ("A server is required to run a Fabric load test.")
                }
            }
            # Advanced mode setup
            elseif ($true -eq $RunBatch) {
                
                # Get the workspace id.
                Add-LogEntry -Thread $null -Iteration $null -MessageType "Information" -MessageText ("Calling API to get workspace id.")
                $WorkspaceID = (Get-FabricWorkspace -Workspace $WorkspaceName).id

                # Get the capacity id.
                Add-LogEntry -Thread $null -Iteration $null -MessageType "Information" -MessageText ("Calling API to get capacity id.")
                ...
                $CapacityRegion = (Get-FabricCapacity -Capacity $CapacityName).region
                ...
                # Perform the necessary capacity related functions (Assign | Scale | Resume).
                if ($true -eq $RunBatch) {
                    try {
                ...
                # Check to be sure the SQL endpoint is accessible. 
                if ($true -eq $RunBatch) {
                    ...
                    do {
                        try {
                            ...
                            $Query = "SELECT REPLACE(REPLACE(REPLACE(@@VERSION, CHAR(9), '|'), CHAR(10), ''), CHAR(13), '') AS version, compatibility_level, collation_name, is_auto_create_stats_on, is_auto_update_stats_on, is_result_set_caching_on, is_vorder_enabled FROM sys.databases WHERE name = '{0}'" -f $ItemName
                            ...
                            $DatabaseVersion                 = $QueryOutput.Dataset.Tables.Rows.version
    ...
    if ($true -eq $RunBatch) {
        ...
        $ParallelThreads = foreach ($Thread in 1..$ThreadCount) {
            ...
            -ArgumentList ($BatchID, $Thread, $ThreadCount, $IterationCount, $Server, $ItemName, $QueryRetryLimit, $QueryDirectory, $StoreQueryResults, $JobFunctions, $DebugMode) `
            -ScriptBlock {
                param(
                    $BatchID,
                    $Thread,
                ...
                $ThreadStartTime = (Get-Date).ToUniversalTime()
                ...
                foreach ($Iteration in 1..$IterationCount) {
                    ...
                    $IterationStartTime = (Get-Date).ToUniversalTime()
                    ...
                    foreach($CurrentQuery in $QueryList) {
                        ...
                        do {
                            try {                                
                                ...
                                if ($true -eq $DebugMode) {
                                    Add-LogEntry -Thread $Thread -Iteration $Iteration -Query $CurrentQuery.BaseName -MessageType "Debug" -MessageText "Currently running in debug mode. No query will be executed."
                                }
                                else {
                                    $QueryOutput = Invoke-FabricSQLCommand -Server $Server -Database $ItemName -Query $Query
                                }
                        ...
                        if($QuerySuccessful) {
                            ...
                            Add-LogEntry -Thread $Thread -Iteration $Iteration -Query $CurrentQuery.BaseName -MessageType "Information" -MessageText ("Searching the message output to look for distributed statement ids has started.")
                            ...
                            Add-LogEntry -Thread $Thread -Iteration $Iteration -Query $CurrentQuery.BaseName -MessageType "Information" -MessageText ("Searching the message output to look for distributed statement ids has ended.")
                    ...
                    $IterationEndTime = (Get-Date).ToUniversalTime()
                    $LocalLogIteration[$IterationID] = @{
                        "IterationID"   = $IterationID
                        "BatchID"       = $BatchID
                        "ThreadID"      = $ThreadID
                ...
                # Update the message in the log.
                $ThreadEndTime = (Get-Date).ToUniversalTime()
                $LocalLogThread[$ThreadID] = @{
                    "ThreadID"      = $ThreadID
                    "BatchID"       = $BatchID
                    "Thread"        = $Thread
    ...
    $BatchEndTime = (Get-Date).ToUniversalTime()

    if (($true -eq $CollectQueryInsights -or $true -eq $CollectCapacityMetrics) -and ($true -eq $RunBatch -or $true -eq $MetricsOnly)) {
        ...
        if ($DistributedStatementCount -gt 0) {
            $DistributedStatementIDList = ($LogStatement.Values.DistributedStatementID).ToUpper()
    ...
    if ($true -eq $CollectQueryInsights -and ($true -eq $RunBatch -or $true -eq $MetricsOnly) -and $DistributedStatementCount -gt 0) {
        if ($true -eq $DebugMode) {
            Add-LogEntry -Thread $null -Iteration $null -MessageType "Debug" -MessageText ("Currently running in debug mode. No query insights information will be collected.")
        }
        else {
            ...
            do {
                ...
                Add-LogEntry -Thread $null -Iteration $null -MessageType "Information" -MessageText ("The expected number of distributed statement ids in query insights is {0} and the current number is {1}." -f $DistributedStatementCount, $QueryInsightsList.Count)
                
                # If the statement count has not been met and the time limit has not expired, wait for a minute and then check again.
                if (($QueryInsightsList.Count -ne $DistributedStatementCount) -and ((Get-Date) -lt $WaitForJobsUntil)) {
                ...
                if ((Get-Date) -gt $WaitForJobsUntil) {
                    Add-LogEntry -Thread $null -Iteration $null -MessageType "Warning" -MessageText ("The batch completed more than {0} minutes ago and the queries have not appeared in query insights. Therefore, query insights data may be incomplete. {1} of {2} found." -f $WaitTimeInMinutesForQueryInsightsData, $QueryInsightsList.Count, $DistributedStatementCount)
                    $ContinueLoop = $false
                }
            } while (
                ($QueryInsightsList.Count -ne $DistributedStatementCount) -and ($true -eq $ContinueLoop)
            )
            
            # Store the query insights results.
            if ($QueryInsightsList.Count -gt 0) {
                $QueryInsights = $QueryInsightsList | Select-Object * -ExcludeProperty ItemArray, Table, RowError, RowState, HasErrors
            }
        }
    }

    # Gather details from capacity metrics. 
    if ($true -eq $CollectCapacityMetrics -and ($true -eq $RunBatch -or $true -eq $MetricsOnly) -and $DistributedStatementCount -gt 0) {
        if ($true -eq $DebugMode) {
            Add-LogEntry -Thread $null -Iteration $null -MessageType "Debug" -MessageText ("Currently running in debug mode. No capacity metrics information will be collected.")
        }
        else {
    ...
    $LogBatch = [ordered]@{
        ...
        "IsAdvancedExecutionMode"      = $(if ($true -eq $AdvancedMode) {$true} else {$false})
    ...
    if ($true -eq $DebugMode) {
        $BatchNameLogFile = ("{0}_DEBUG" -f $BatchNameLogFile) 
    }
